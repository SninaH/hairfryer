...
from src.video_utils.image_to_number import get_number, light_type
...
def get_raw_data_from_frame(frame_path, fields):
    # size = 8
...
def get_data_from_frames(path, fields):
    # values = dict()
    values = []
    ...
    for frame in frames:
        ...
        value = get_raw_data_from_frame(frame_path, fields)
        values.append(value)
...
async def get_data(input_path, video_name, fields):
    ...
    for met in cleaned_data:
        if met == 0:
            continue
        throws.append({
            "pins_fallen": cleaned_data[met][1],
            "pins_hit": cleaned_data[met][3:]
        })
    return total_throws, total_pins_fallen, throws
